...
const LoginView = () => { 
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await signIn("credentials", { redirect: false, email: form.email.value, password: form.password.value, callbackUrl });
            if (!res?.error) {
                setIsLoading(false);
                form.reset();
                push(callbackUrl);    
        ...
        <div className={styles.login}>
            ...
            <div className={styles.login__form}>
                ...
                <div className={styles.login__form__other}>
                    <button type='button' onClick={() => signIn('google', { callbackUrl, redirect: false })} className={styles.login__form__other__button}><i className='bx bxl-google'></i>Login with Google</button>
                </div>
            </div>            
